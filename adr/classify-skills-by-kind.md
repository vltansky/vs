...
Skills in `./skills/` vary by role. Some start a full workflow (`vs-shape-it`, `vs-ship-it`), and others own one bounded job: a workflow phase, a behavior mode, a decision helper, or a meta workflow (`vs-tdd`, `vs-roast-review`, `autoanswer`, `caveman`, `vs-retro`, `vs-try-skill`). Nothing in the codebase currently says this out loud, which makes three things harder:
...
3. **Documentation** — we keep saying "use `/vs-pushback`, which is itself called from `/vs-shape-it`" in prose, with no shared vocabulary for which skill is the caller and which is the callee.
...
- **Three levels.** Rejected — the proposed L3 bucket mixed unlike things: real user-invocable workflows (`vs-retro`, `vs-try-skill`), behavior modes (`caveman`), decision helpers (`autoanswer`), and shared references (`vs-internal-shared`). That made L3 less useful than a broader L2 plus explicit descriptions.
