...
export const vsLayoutCss = `
  ...
  @media screen {
    .htmdx-app[data-htmdx-layout^='vs'] .htmdx-mermaid svg :is(.node, .statediagram-state, .actor) :is(rect, circle, ellipse, polygon, path):not(g.basic.label-container path),
    ...
    import(globalThis.Htmdx.DEFAULT_MERMAID_SRC)
      .then((mod) => {
        ...
        const repaint = () => {
          ...
          if (!host) return;
          if (!blocks.some((block) => block.querySelector('svg rect.basic.label-container'))) return;
          globalThis.Htmdx.resolveSource(host)
            .then(({ ok, source }) => {
              ...
              blocks.forEach((block, index) => {
                ...
                if (!svg || !svg.querySelector('rect.basic.label-container')) return;
