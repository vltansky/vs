...
export const vsCatalogFactory = (React, css) => {
    ...
    wrap(
      ...
      h(
        ...
        rows(body).flatMap((row, index) => {
          ...
          const stage = h(
            'div',
            { key: 'stage' + index, className: 'vs-flow-stage rounded-lg border bg-card px-4 py-3' },
            h(
              'div',
              { className: 'font-mono text-xs font-bold tracking-widest text-card-foreground' },
              index + 1 + ' · ' + plain(label).toUpperCase(),
            ),
  ...
  const components = [
    ...
    },
    {
      ...
      body: 'markdown',
      purpose:
        "Show a linear pipeline, rollout, or lifecycle as stage cards joined by arrows, written as '- Stage: detail' rows in order. Three to six stages read best; the strip collapses to a column on narrow screens. It renders straight-line flow only - branching, cycles, or fan-out belong in a mermaid diagram.",
      example:
