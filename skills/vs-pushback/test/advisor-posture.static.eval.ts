...
describe('pushback: proportional review and verification', () => {
  ...
  it('closes interactive review with vs-eli5 and skips it when composed', () => {
    ...
    expect(SKILL).toMatch(/2\. The eli5 file path \(or .eli5 saved.\)/);
    expect(SKILL).not.toMatch(/2\. The opened eli5/);
    expect(SKILL).toMatch(/Do not auto-open/i);
    expect(SKILL).toMatch(/Do not run `open` or `xdg-open`/i);
    expect(SKILL).toMatch(/3\. `Handoff:/);
    expect(SKILL).toMatch(/4\. One `Your action`/);
    expect(SKILL).toMatch(/NOT_READY: rework only/i);
    expect(SKILL).toMatch(/READY or READY_WITH_RISKS: approve/i);
    expect(SKILL).toMatch(/do not offer approve or `\/vs-build-it`/i);
    ...
    expect(SKILL).not.toMatch(/One `Your action` confirm line/);
    expect(SKILL).toMatch(/Write the `Stress-Test Assessment`.*to the report file/s);
