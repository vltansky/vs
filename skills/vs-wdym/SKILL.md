---
name: vs-wdym
description: "User-invoked repair for when the agent's explanation did not land. Use when the user types `/vs-wdym`, says `what do you mean?`, `wait, what?`, `lost me`, or asks for the previous point to be explained again; re-pitch the relevant context in plain English, shorter and clearer without dropping the premise."
...
- Treat `/vs-wdym` as a comprehension signal, not a request for a terse summary.
...
- A second `/vs-wdym` works as another repair rather than degrading into fragments.

## Boundary

Use `/vs-wdym` at any point, including inside another skill. It repairs one
