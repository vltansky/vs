...
**Output**: A design brief that can be handed off to /vs-roast-ui craft, or directly to /vs-roast-ui for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Before the brief is final, run a distinctiveness check: can this concept only
belong to this product, audience, and moment? If the visual direction could be
swapped onto a generic SaaS, portfolio, or dashboard prompt without anyone
noticing, revise the thesis, anchor references, or signature move before asking
for confirmation.
...
Based on the brief, list which vs reference files would be most valuable during implementation (e.g., layout.md for complex layouts, animate.md for animated features, interaction-design.md for form-heavy features, typeset.md for typography-driven pages, colorize.md for color-led brands).
...
Once confirmed, the brief is complete. The user can now hand it to /vs-roast-ui, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /vs-roast-ui craft instead, which runs this command internally.)
