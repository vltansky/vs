 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /roast-ui audit.
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /roast-ui ${command}.`;
...
This is a pinned shortcut for \`/roast-ui ${command}\`.

Invoke /roast-ui ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /roast-ui ${command} to access it.`);
  } else {
