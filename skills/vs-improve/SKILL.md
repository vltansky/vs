...
reviewed and the verdict is reported.
...
Apply [`vs-ponytail`](../vs-ponytail/SKILL.md) in composed mode to each
surviving proposal. Prefer the smallest independently useful plan that
...
- With a focus argument (e.g. `security`, `vs-perf`, `tests`) → run Recon, then audit only that category, then plan.
