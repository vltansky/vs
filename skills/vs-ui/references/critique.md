...
2. **Compute the slug**:
   ```bash
   node $VS_UI_SKILL_DIR/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
   Keep it. If the command exits non-zero, skip persistence and trend for this run, but continue the critique.
3. **Read `.vs/critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.
...
If the user supplied reference screenshots or mocks, also run a visual fidelity
pass using [verdict.md](verdict.md) after Assessment A and B. Keep it separate
from the heuristic critique: fidelity to a reference answers "does this match
the target?", while critique answers "is this good for the user?"
...
- If `spawn_agent` is exposed but the user did not explicitly allow sub-agents, ask exactly once: "VS critique is designed to run two independent sub-agents for an unanchored assessment. May I use sub-agents for this critique?" Then stop until the user answers.
...
Prefer `fork_turns: "none"` with self-contained prompts containing cwd, target, live URL, references, product context, and output contract. If using `fork_turns: "all"`, omit `model` and `reasoning_effort`.
</codex>
...
CLI scan:
```bash
node $VS_UI_SKILL_DIR/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node $VS_UI_SKILL_DIR/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `vs` console messages, then stop the live server.
5. For multi-view targets, inject on 3-5 representative pages.

<codex>
Codex Browser note: Use the Browser skill. Do not spend a Browser attempt on `file://`. Only call `visibility.set(true)` after mutable script injection is confirmed for the `[Human]` overlay path; verify with `get()`. Use `tab.dev.logs({ filter: "vs" })` for console results. Its Playwright `evaluate(...)` surface is read-only; do not rely on it for mutation.
</codex>
...
<codex>
Codex final-answer note: `$vs-ui critique` produces a report artifact, so the final chat response should intentionally exceed the usual concise close-out style. Do not title the final response "Critique Summary" unless the user explicitly asked for a summary.
</codex>
...
**Reference fidelity** (if references were provided): Include the `verdict`
score, status, and top mismatches. Do not let a high fidelity score hide UX or
accessibility problems; exact copies can still be bad interfaces.
...
- **Suggested command**: Which command could address this (from: craft, shape, init, document, extract, critique, audit, verdict, polish, bolder, quieter, distill, harden, onboard, animate, colorize, typeset, layout, delight, overdrive, clarify, adapt, optimize, live)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `the agent config file` contains a `## Design Context` section from `vs init`, also generate 1-2 project-specific personas from the audience/brand info.
...
Once the report above is finalized, write it to `.vs/critique/` so the user can refer back, and so `/vs-ui polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `VS_CRITIQUE_META` (JSON), then run the write command:
   ```bash
   VS_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node $VS_UI_SKILL_DIR/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node $VS_UI_SKILL_DIR/scripts/critique-storage.mjs trend <slug> 5
   ```
...
- Only recommend commands from: craft, shape, init, document, extract, critique, audit, verdict, polish, bolder, quieter, distill, harden, onboard, animate, colorize, typeset, layout, delight, overdrive, clarify, adapt, optimize, live
...
- End with `/vs-ui polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `/vs-ui critique` after fixes to see your score improve.
...
If `the agent config file` contains a `## Design Context` section (generated by `vs init`), derive 1–2 additional personas from the audience and brand information:
