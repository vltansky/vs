...
```bash
node $VS_UI_SKILL_DIR/scripts/live.mjs
```
...
```
LOOP:
  node $VS_UI_SKILL_DIR/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node $VS_UI_SKILL_DIR/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-status.mjs
node $VS_UI_SKILL_DIR/scripts/live-resume.mjs --id SESSION_ID
node $VS_UI_SKILL_DIR/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node $VS_UI_SKILL_DIR/scripts/live-server.mjs stop
```
...
```bash
node $VS_UI_SKILL_DIR/scripts/detect-csp.mjs
```
