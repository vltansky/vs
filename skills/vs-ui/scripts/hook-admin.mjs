#!/usr/bin/env node
/**
 * `/vs-ui hooks <on|off|status|reset>` — manage the design hook
...
const VS_HOOK_COMMAND_MARKERS = [
  'skills/vs-ui/scripts/hook-probe.mjs',
  'skills/vs-ui/scripts/hook.mjs',
  'skills/vs-ui/scripts/hook-before-edit.mjs',
  'skills/vs-ui/scripts/hook-after-edit.mjs',
  'skills/vs-ui/scripts/hook-stop.mjs',
];
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/vs-ui/scripts/hook.mjs"',
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "$(git rev-parse --show-toplevel)/.agents/skills/vs-ui/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
  ...
  },
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/vs-ui/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
...
function writeConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive a `/vs-ui hooks` edit.
...
function mergeConfig(existing) {
  // Persist the full shape so /vs-ui hooks edits leave a complete file
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. /vs-ui hooks ignore-rule side-tab');
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error('overused-font is value-specific by default. Use /vs-ui hooks ignore-value overused-font <font> for a confirmed font, or /vs-ui hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.');
  }
  const config = mergeConfig(readRawConfig(cwd));
...
function addIgnoreFile(cwd, glob) {
  if (!glob) throw new Error('Pass a glob, e.g. /vs-ui hooks ignore-file "src/legacy/**"');
  const config = mergeConfig(readRawConfig(cwd));
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error('Pass a rule id and value, e.g. /vs-ui hooks ignore-value overused-font Inter');
  }
