 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /vs-ui audit.
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /vs-ui ${command}.`;
...
This is a pinned shortcut for \`/vs-ui ${command}\`.

Invoke /vs-ui ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /vs-ui ${command} to access it.`);
  } else {
