...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run /vs-ui audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0
    ? `... and ${remaining} more (see /vs-ui audit).`
    : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see /vs-ui audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) => [
    header,
    ...linesArr,
    ...(omitted ? ['... and more (see /vs-ui audit).'] : []),
    '',
    footer,
  ].join('\n');
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  while (assembled.length > maxChars && working.length > 1) {
    working.pop();
    moreText = '... and more (see /vs-ui audit).';
...
function formatFindingIgnoreCommand(finding) {
  ...
  return `/vs-ui hooks ignore-value ${rule} ${valueArg} --shared --reason ${reason}`;
}
...
function directiveFooter(display, opts = {}) {
  const ignoreFileCommand = `/vs-ui hooks ignore-file ${quoteCommandArg(display)}`;
  const fileIgnoreGuidance = opts.grouped
    ? 'run `/vs-ui hooks ignore-file <path>` for the specific file'
  ...
  return [
    ...
    '',
    `Do not change intentional design just to satisfy the hook. Do not add source comments such as \`vs: ignore\`; those pollute the code and do not suppress hook findings. Persist hook ignores only after the user explicitly confirms the finding is intentional. Prefer the narrowest persisted exception: run the exact \`/vs-ui hooks ignore-value ... --shared\` command shown next to a value-specific finding. For \`overused-font\`, use \`ignore-value\` for a specific font and use \`/vs-ui hooks ignore-rule overused-font --all-values\` only when the user asks to ignore overused fonts generally. For file-specific findings without an ignore-value command, ${fileIgnoreGuidance}; use \`/vs-ui hooks ignore-rule <id>\` only when the user asks to suppress the whole non-value-specific rule. Run /vs-ui audit for the full pass.`,
  ].join('\n');
}
